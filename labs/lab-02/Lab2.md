# PART 1:

### 1.2
https://github.com/gabeorlanski/lab2part1

### 1.6 
I could not get GitK to work on WSL even with the X server. So instead I used loaded the repository in GitKraken and screenshotted the branches.

![Git Branches](https://github.com/gabeorlanski/oss-repo-template/blob/master/labs/lab-02/branches.PNG)

# PART 2:

### 2.1

[Spoon Knife](https://github.com/gabeorlanski/Spoon-Knife)

### 2.4 

![Git Branching Tutorial](https://github.com/gabeorlanski/oss-repo-template/blob/master/labs/lab-02/git%20branching.PNG)

# PART 3:

### 3.1

[PullReq Fork](https://github.com/gabeorlanski/PullReq)

### 3.2

![Git Diff](https://github.com/gabeorlanski/oss-repo-template/blob/master/labs/lab-02/git_diff.PNG)

### 3.3

![Git Diff](https://github.com/gabeorlanski/oss-repo-template/blob/master/labs/lab-02/git_tag.PNG)

### 3.5 

[Table's Repo](https://github.com/rcos/CSCI-4470-OpenSource/blob/master/Modules/02.Git/Lab-Git.md)

